...
const StadiumMap = ({ zones }) => {
  ...
  return (
    <div id="stadium-map" className="glass-panel p-6 flex flex-col h-[500px]">
      ...
      <div className="flex-1 relative bg-slate-50 rounded-xl overflow-hidden border border-slate-200 p-4">
         {/* Field Mock */}
         <div className="absolute top-1/2 left-1/2 -translate-x-1/2 -translate-y-1/2 w-1/3 h-1/2 border-2 border-brand/20 rounded-[50px] bg-brand/5 flex items-center justify-center">
         ...
         {Object.keys(zones).map(key => {
            ...
            return (
               <button 
                  ...
                  className={clsx(
                     "absolute -translate-x-1/2 -translate-y-1/2 px-3 py-1.5 rounded-full text-xs font-bold transition-all hover:scale-110",
         ...
         {activeZone && (
            <div className="absolute bottom-4 left-4 right-4 bg-white/95 backdrop-blur-sm p-4 rounded-xl shadow-lg border border-slate-200 flex items-start gap-4 animate-fade-in z-10">
